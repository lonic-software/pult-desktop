...
/// `AppHandle`-free so it's unit-testable); once it returns, this starts
/// tailing the run's journal (`journal::tail_run`), which is what actually
/// delivers `Line`/`Step`/`Progress`/`Status`/`Exit` events to the frontend
/// on the `pult://run-output` channel — this command returns as soon as the
/// tail has started, not when the run finishes.
///
...
pub async fn run_command(
    ...
    journal::tail_run(app.clone(), tails.inner().clone(), path, run_id);
    Ok(())
}
